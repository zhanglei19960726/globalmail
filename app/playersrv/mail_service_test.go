...
func TestPlayerMailServiceListGlobalMailsMergesStates(t *testing.T) {
	now := time.Now().UTC()
	...
	service := NewPlayerMailService(repo, localCache)
...
func TestPlayerMailServiceListGlobalMailsSkipsDeletedState(t *testing.T) {
	now := time.Now().UTC()
	repo := &fakeMailRepository{
		mails: []globalmail.GlobalMail{
			{
...
func TestPlayerMailServiceClaimGlobalMailIsIdempotent(t *testing.T) {
	now := time.Now().UTC()
	repo := &fakeMailRepository{
		mails: []globalmail.GlobalMail{
			{
...
func TestPlayerMailServiceClaimRetryUsesRewardLedgerIdempotency(t *testing.T) {
	now := time.Now().UTC()
	repo := &fakeMailRepository{
		mails: []globalmail.GlobalMail{
			{
	...
	for _, grant := range repo.grants {
		if grant.Status != "succeeded" || grant.UpdateTime.IsZero() {
			t.Fatalf("expected reward ledger status and update time, got %+v", grant)
		}
	}
}

func TestPlayerMailServiceClaimPassesLootToRewardRepository(t *testing.T) {
	now := time.Now().UTC()
	repo := &fakeMailRepository{
		mails: []globalmail.GlobalMail{{
			ID:         1,
			Status:     globalmail.MailStatusPublished,
			Loots:      []byte(`[{"item_id":100,"count":1},{"item_id":200,"count":2}]`),
			StartTime:  now.Add(-time.Hour),
			ExpireTime: now.Add(time.Hour),
		}},
		states: map[int64]globalmail.UserGlobalMailState{},
	...
	grant := repo.grants["10001:1:1"]
	if string(grant.Loot) != `{"item_id":200,"count":2}` {
		t.Fatalf("expected claimed loot payload, got %s", string(grant.Loot))
	}
}

func TestPlayerMailServiceDeleteGlobalMailWritesDeletedState(t *testing.T) {
	now := time.Now().UTC()
	repo := &fakeMailRepository{
		mails: []globalmail.GlobalMail{
			{
...
func TestPlayerMailServiceDeletedStateIsNotOverwritten(t *testing.T) {
	now := time.Now().UTC()
