...
type CommandRouter interface {
	Resolve(ctx context.Context, uid int64) (PlayerServerInstance, error)
...
type GameCommandClientFactory interface {
	ClientFor(ctx context.Context, instance PlayerServerInstance) (GameCommandClient, error)
}
...
func (f *GRPCGameCommandClientFactory) ClientFor(ctx context.Context, instance PlayerServerInstance) (GameCommandClient, error) {
	f.mu.Lock()
	defer f.mu.Unlock()

	if conn, ok := f.conns[instance.GrpcAddr]; ok {
		return rpc.NewPlayerCommandServiceClient(conn), nil
	}
