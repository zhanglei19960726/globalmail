...
type fakePlayerServerProvider struct {
	instances []PlayerServerInstance
	calls     int
}

func (f *fakePlayerServerProvider) ListReadyPlayerServers(context.Context) ([]PlayerServerInstance, error) {
	f.calls++
...
func TestRouteServiceUsesRedisRouteBeforeHashing(t *testing.T) {
	...
	provider := &fakePlayerServerProvider{
		instances: []PlayerServerInstance{{InstanceID: "game-1"}},
	}
...
func TestRouteServiceClearRemovesSessionAndRedisRoute(t *testing.T) {
	store := &fakeRouteStore{routes: map[int64]routing.SrvRouter{}}
	provider := &fakePlayerServerProvider{instances: []PlayerServerInstance{{InstanceID: "game-1"}}}
	service := NewRouteService(store, provider, time.Minute, 100)
	service.session.Set(10001, PlayerServerInstance{InstanceID: "game-1"})
