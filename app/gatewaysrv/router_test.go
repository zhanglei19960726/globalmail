...
func TestConsistentHashRouterRoutesSameUIDToSameInstance(t *testing.T) {
	router := NewConsistentHashRouter([]PlayerServerInstance{
...
func TestConsistentHashRouterReturnsErrorWithoutInstances(t *testing.T) {
	...
	if !errors.Is(err, ErrNoPlayerServer) {
		t.Fatalf("expected ErrNoPlayerServer, got %v", err)
	}
}

func TestConsistentHashRouterMinimizesMovementWhenAddingNode(t *testing.T) {
	before := NewConsistentHashRouter([]PlayerServerInstance{
		{InstanceID: "game-1"},
		{InstanceID: "game-2"},
		{InstanceID: "game-3"},
	}, 200)
	after := NewConsistentHashRouter([]PlayerServerInstance{
		{InstanceID: "game-1"},
		{InstanceID: "game-2"},
		{InstanceID: "game-3"},
		{InstanceID: "game-4"},
