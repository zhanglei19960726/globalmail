...
func main() {
	...
	relay := globalmail.NewOutboxRelay(
		...
		globalmail.WithOutboxRelayLockTTL(cfg.MailRelay.LockTTL.Duration),
		globalmail.WithOutboxRelayMaxRetries(cfg.MailRelay.MaxRetries),
		globalmail.WithOutboxRelayMetrics(metrics.NewExpvarMetrics("mailrelaysrv_globalmail")),
	)
	flushInterval := cfg.MailRelay.FlushInterval.Duration
	if *interval > 0 {
		flushInterval = *interval
	}
	fetchLimit := cfg.MailRelay.FetchLimit
	if *limit > 0 {
		fetchLimit = *limit
	}
	worker := mailrelaysrv.NewWorker(relay, fetchLimit, flushInterval, log.Default())
